...
const transformWithPropDefinition = (propValue, propDefinition) => {
  if (propDefinition.valueType === ValueTypes.value) {
    ...
    isArray(propValue) &&
    propDefinition.collectionValidator.indexOf(isOfTypeArray) > -1
  ) {
    const valueValidator = propDefinition.validators;
    if (valueValidator.valueType === ValueTypes.value) {
      return propValue.map((value) => valueValidator.transform(value));
    } else {
      return propValue.map((value) => transformObject(value, valueValidator));
    }
  } else if (
    isPureObject(propValue) &&
    propDefinition.collectionValidator.indexOf(isOfTypeObject) > -1
  ) {
    const ret = {};
